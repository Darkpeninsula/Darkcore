    ...
    struct instance_onyxias_lair_InstanceMapScript : public InstanceScript
    {
        ...
        void FloorEruption(uint64 floorEruptedGUID)
        {
            if (GameObject* floorEruption = instance->GetGameObject(floorEruptedGUID))
            {
                ...
                Darkcore::GameObjectInRangeCheck check(floorEruption->GetPositionX(), floorEruption->GetPositionY(), floorEruption->GetPositionZ(), 15);
                Darkcore::GameObjectListSearcher<Darkcore::GameObjectInRangeCheck> searcher(floorEruption, nearFloorList, check);
