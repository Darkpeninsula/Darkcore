    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (BfTB)
        {
            ...
            for (uint8 i = 0; i<gy.size(); i++)
            {
                ...
                {
                   player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GetCoreStringForDBCLocale(((BfGraveYardTB*)gy[i])->GetTextId()), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+i);
                }
            }
        }
...
class npc_tol_barad_battlemage : public CreatureScript
{
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (BfTB)
        {
            if (BfTB->IsWarTime())
            {
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GetCoreStringForDBCLocale(TB_NPCQUEUE_TEXTOPTION_JOIN), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
