    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (BfWG)
        {
            ...
            for (uint8 i = 0; i < gy.size(); i++)
            {
                ...
                {
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GetCoreStringForDBCLocale(((BfGraveYardWG *) gy[i])->GetTextId()), GOSSIP_SENDER_MAIN,
  ...
  public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (BfWG)
        {
            if (BfWG->IsWarTime())
            {
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GetCoreStringForDBCLocale(WG_NPCQUEUE_TEXTOPTION_JOIN), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
