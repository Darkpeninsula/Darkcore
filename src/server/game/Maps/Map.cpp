...
void Map::SwitchGridContainers(Creature* obj, bool on)
{
    ...
    CellCoord p = Darkcore::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
    if (!p.IsCoordValid())
    {
...
void Map::EnsureGridCreated(const GridCoord &p)
{
    ...
    {
        DARKCORE_GUARD(ACE_Thread_Mutex, Lock);
        if (!getNGrid(p.x_coord, p.y_coord))
        {
...
{
    CellCoord cellCoord = Darkcore::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
...
bool Map::AddToMap(T *obj)
{
    ...
    CellCoord cellCoord = Darkcore::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
...
void Map::VisitNearbyCellsOf(WorldObject* obj, TypeContainerVisitor<Darkcore::ObjectUpdater, GridTypeMapContainer> &gridVisitor, TypeContainerVisitor<Darkcore::ObjectUpdater, WorldTypeMapContainer> &worldVisitor)
{
...
void Map::Update(const uint32 t_diff)
{
    ...
    Darkcore::ObjectUpdater updater(t_diff);
    // for creature
    TypeContainerVisitor<Darkcore::ObjectUpdater, GridTypeMapContainer  > grid_object_update(updater);
    // for pets
    TypeContainerVisitor<Darkcore::ObjectUpdater, WorldTypeMapContainer > world_object_update(updater);
    ...
    for (GridRefManager<NGridType>::iterator i = GridRefManager<NGridType>::begin(); i != GridRefManager<NGridType>::end(); ++i)
    {
        ...
        for (uint32 x = cell_min.x_coord; x < cell_max.x_coord; ++x)
        {
            for (uint32 y = cell_min.y_coord; y < cell_max.y_coord; ++y)
            {
                ...
                Darkcore::DelayedUnitRelocation cell_relocation(cell, pair, *this, MAX_VISIBILITY_DISTANCE);
                TypeContainerVisitor<Darkcore::DelayedUnitRelocation, GridTypeMapContainer  > grid_object_relocation(cell_relocation);
                TypeContainerVisitor<Darkcore::DelayedUnitRelocation, WorldTypeMapContainer > world_object_relocation(cell_relocation);
...
{
    cell.SetNoCreate();
    Darkcore::VisibleChangesNotifier notifier(*obj);
    TypeContainerVisitor<Darkcore::VisibleChangesNotifier, WorldTypeMapContainer > player_notifier(notifier);
...
{
    Darkcore::VisibleNotifier notifier(*player);

    cell.SetNoCreate();
    TypeContainerVisitor<Darkcore::VisibleNotifier, WorldTypeMapContainer > world_notifier(notifier);
    TypeContainerVisitor<Darkcore::VisibleNotifier, GridTypeMapContainer  > grid_notifier(notifier);
...
bool Map::ActiveObjectsNearGrid(NGridType const& ngrid) const
{
    ...
    for (MapRefManager::const_iterator iter = m_mapRefManager.begin(); iter != m_mapRefManager.end(); ++iter)
    {
        ...
        CellCoord p = Darkcore::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
...
void Map::AddToActive(Creature* c)
{
    ...
    if (!c->isPet() && c->GetDBTableGUIDLow())
    {
        ...
        GridCoord p = Darkcore::ComputeGridCoord(x, y);
        if (getNGrid(p.x_coord, p.y_coord))
            getNGrid(p.x_coord, p.y_coord)->incUnloadActiveLock();
        else
        {
            GridCoord p2 = Darkcore::ComputeGridCoord(c->GetPositionX(), c->GetPositionY());
