...
BattlegroundBG::BattlegroundBG()
{
    ...
    _StartMessageIds[BG_STARTING_EVENT_SECOND] = LANG_BG_BG_START_ONE_MINUTE;
    _StartMessageIds[BG_STARTING_EVENT_THIRD]  = LANG_BG_BG_START_HALF_MINUTE;
    _StartMessageIds[BG_STARTING_EVENT_FOURTH] = LANG_BG_BG_HAS_BEGUN;
}
...
void BattlegroundBG::EventPlayerClickedOnFlag(Player* source, GameObject* /*target_obj*/)
{
    ...
    GameObject* obj = GetBgMap()->GetGameObject(_BgObjects[node*8+5]);
    while ((node < BG_BG_DYNAMIC_NODES_COUNT) && ((!obj) || (!source->IsWithinDistInMap(obj,10))))
    {
        ++node;
        obj = GetBgMap()->GetGameObject(_BgObjects[node*8+BG_BG_OBJECT_AURA_CONTESTED]);
    }
