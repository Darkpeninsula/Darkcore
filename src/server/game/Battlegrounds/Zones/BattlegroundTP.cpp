...
{
    _BgObjects.resize(BG_TP_OBJECT_MAX);
    _BgCreatures.resize(BG_CREATURES_MAX_TP);

    _StartMessageIds[BG_STARTING_EVENT_FIRST]  = LANG_BG_TP_START_TWO_MINUTES;
...
void BattlegroundTP::EventPlayerClickedOnFlag(Player* Source, GameObject* target_obj)
{
    ...
    if (Source->GetTeam() == HORDE && this->GetFlagState(ALLIANCE) == BG_TP_FLAG_STATE_ON_BASE
        && this->_BgObjects[BG_TP_OBJECT_A_FLAG] == target_obj->GetGUID())
    {
    ...
    if (Source->GetTeam() == ALLIANCE && this->GetFlagState(HORDE) == BG_TP_FLAG_STATE_ON_BASE
        && this->_BgObjects[BG_TP_OBJECT_H_FLAG] == target_obj->GetGUID())
    {
