...
struct BfTBWorkShopData
{
    ...
    uint32 m_NameId;                          // Id of core_string witch contain name of this node, using for alert message
    ...
    void ChangeControl(uint8 team, bool init /* for first call in setup*/)
    {
        switch (team)
        {
            case BATTLEFIELD_TB_TEAM_NEUTRAL:
            {
                ...
                m_TB->SendWarningToAllInZone(m_NameId),
                    sObjectMgr->GetCoreStringForDBCLocale(m_TeamControl ? BATTLEFIELD_TB_TEXT_ALLIANCE : BATTLEFIELD_TB_TEXT_HORDE);
                break;
            }
            case BATTLEFIELD_TB_TEAM_ALLIANCE:
            {
                ...
                if (!init)
                    m_TB->SendWarningToAllInZone(m_NameId),
                    sObjectMgr->GetCoreStringForDBCLocale(BATTLEFIELD_TB_TEXT_ALLIANCE);
            ...
            case BATTLEFIELD_TB_TEAM_HORDE:
            {
                ...
                if (!init)
                    m_TB->SendWarningToAllInZone(m_NameId),
                        sObjectMgr->GetCoreStringForDBCLocale(BATTLEFIELD_TB_TEXT_HORDE);
