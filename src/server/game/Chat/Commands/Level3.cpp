...
bool ChatHandler::HandleCooldownCommand(const char *args)
{
    ...
    else
    {
        ...
        {
            PSendSysMessage(LANG_UNKNOWN_SPELL, target == _session->GetPlayer() ? GetCoreString(LANG_YOU) : tNameLink.c_str());
            SetSentErrorMessage(true);
            return false;
        }

        target->RemoveSpellCooldown(spell_id, true);
        PSendSysMessage(LANG_REMOVE_COOLDOWN, spell_id, target == _session->GetPlayer() ? GetCoreString(LANG_YOU) : tNameLink.c_str());
   }
    return true;
}
...
bool ChatHandler::HandleResetAllCommand(const char * args)
{
    ...
    DARKCORE_READ_GUARD(HashMapHolder<Player>::LockType, *HashMapHolder<Player>::GetLock());
...
bool ChatHandler::HandleRespawnCommand(const char* /*args*/)
{
    ...
    Darkcore::WorldObjectWorker<Darkcore::RespawnDo> worker(player, u_do);
