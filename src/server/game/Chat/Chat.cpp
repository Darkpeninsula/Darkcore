...
{
    return _session->GetCoreString(entry);
}
...
GameObject* ChatHandler::GetObjectGlobalyWithGuidOrNearWithDbGuid(uint32 lowguid, uint32 entry)
{
    ...
    if (!obj && sObjectMgr->GetGOData(lowguid))                   // guid is DB guid of object
    {
        ...
        Darkcore::GameObjectWithDbGUIDCheck go_check(*player, lowguid);
        Darkcore::GameObjectSearcher<Darkcore::GameObjectWithDbGUIDCheck> checker(player, obj, go_check);
