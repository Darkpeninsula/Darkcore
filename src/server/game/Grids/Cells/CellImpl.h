...
#ifndef DARKCORE_CELLIMPL_H
#define DARKCORE_CELLIMPL_H
...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellCoord center = Darkcore::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

    CellCoord centerX = Darkcore::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY = Darkcore::ComputeCellCoord(x + radius, y + radius).normalize();
