...
// such checks will speedup darkcore change amount/send for client operations
...
void AuraEffect::HandleFeignDeath(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        */
        ...
        UnitList targets;
        Darkcore::AnyUnfriendlyUnitInObjectRangeCheck u_check(target, target, target->GetMap()->GetVisibilityRange());
        Darkcore::UnitListSearcher<Darkcore::AnyUnfriendlyUnitInObjectRangeCheck> searcher(target, targets, u_check);
            ...
            switch (GetSpellInfo()->Id)
            {
                ...
                case 51690:
                {
                    ...
                    UnitList targets;
                    {
                        ...
                        CellCoord p(Darkcore::ComputeCellCoord(target->GetPositionX(), target->GetPositionY()));
                        Cell cell(p);

                        Darkcore::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck u_check(target, radius);
                        Darkcore::UnitListSearcher<Darkcore::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck> checker(target, targets, u_check);

                        TypeContainerVisitor<Darkcore::UnitListSearcher<Darkcore::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck>, GridTypeMapContainer > grid_object_checker(checker);
                        TypeContainerVisitor<Darkcore::UnitListSearcher<Darkcore::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck>, WorldTypeMapContainer > world_object_checker(checker);
