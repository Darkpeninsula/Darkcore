...
{
    public:
        DarkcoreStringTextBuilder(WorldObject* obj, ChatMsg msgtype, int32 id, uint32 language, uint64 targetGUID) : _source(obj), _msgType(msgtype), _textId(id), _language(language), _targetGUID(targetGUID)
        {
        }

        size_t operator()(WorldPacket* data, LocaleConstant locale) const
        {
            std::string text = sObjectMgr->GetCoreString(_textId, locale);
...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_FLEE_FOR_ASSIST:
        {
            ...
            {
                DarkcoreStringTextBuilder builder(me, CHAT_MSG_MONSTER_EMOTE, LANG_FLEE, LANG_UNIVERSAL, 0);
...
ObjectList* SmartScript::GetWorldObjectsInDist(float dist)
{
    ...
    if (obj)
    {
        Darkcore::AllWorldObjectsInRange u_check(obj, dist);
        Darkcore::WorldObjectListSearcher<Darkcore::AllWorldObjectsInRange> searcher(obj, *targets, u_check);
...
Unit* SmartScript::DoSelectLowestHpFriendly(float range, uint32 MinHPDiff)
{
    ...
    Darkcore::MostHPMissingInRange u_check(me, range, MinHPDiff);
    Darkcore::UnitLastSearcher<Darkcore::MostHPMissingInRange> searcher(me, unit, u_check);

    TypeContainerVisitor<Darkcore::UnitLastSearcher<Darkcore::MostHPMissingInRange>, GridTypeMapContainer >  grid_unit_searcher(searcher);
...
void SmartScript::DoFindFriendlyCC(std::list<Creature*>& _list, float range)
{
    ...
    Darkcore::FriendlyCCedInRange u_check(me, range);
    Darkcore::CreatureListSearcher<Darkcore::FriendlyCCedInRange> searcher(me, _list, u_check);

    TypeContainerVisitor<Darkcore::CreatureListSearcher<Darkcore::FriendlyCCedInRange>, GridTypeMapContainer >  grid_creature_searcher(searcher);
...
void SmartScript::DoFindFriendlyMissingBuff(std::list<Creature*>& list, float range, uint32 spellid)
{
    ...
    Darkcore::FriendlyMissingBuffInRange u_check(me, range, spellid);
    Darkcore::CreatureListSearcher<Darkcore::FriendlyMissingBuffInRange> searcher(me, list, u_check);

    TypeContainerVisitor<Darkcore::CreatureListSearcher<Darkcore::FriendlyMissingBuffInRange>, GridTypeMapContainer >  grid_creature_searcher(searcher);
