        ...
        for (GroupReference* itr = _group->GetFirstMember(); itr != NULL; itr = itr->next())
            if (Player* member = itr->getSource())
                if (member->isAlive() && member->IsAtGroupRewardDistance(_victim))
                {
                    ...
                    // 2.4. _maxNotGrayMember - maximum level of alive group member within reward distance,
                    //      for whom victim is not gray;
                    uint32 grayLevel = Darkcore::XP::GetGrayLevel(lvl);
...
inline void KillRewarder::_InitXP(Player* player)
{
    ...
    if (_isBattleGround || (!_isPvP && !_killer->GetVehicle()))
        _xp = Darkcore::XP::Gain(player, _victim);
}
    ...
    if (_maxLevel)
    {
        ...
        if (!_isBattleGround || _xp)
        {
            ...
            if (!_isBattleGround)
            {
                ...
                _groupRate = Darkcore::XP::xp_in_group_rate(_count, isRaid);
            }
...
void Player::Regenerate(Powers power)
{
    ...
    switch (power)
    {
        case POWER_MANA:
        {
            ...
            if (recentCast) // Darkcore Updates Mana in intervals of 2s, which is correct
...
void Player::removeSpell(uint32 spell_id, bool disabled, bool learn_low_rank)
{
    ...
    if (uint32 prev_id = sSpellMgr->GetPrevSpellInChain(spell_id))
    {
        ...
        if (talentCosts)
        {
            // I cannot see why the core has these lines.
            //if (learn_low_rank)
...
void Player::UpdateCombatSkills(Unit* victim, WeaponAttackType attType, bool defence)
{
    ...
    uint8 greylevel = Darkcore::XP::GetGrayLevel(plevel);
...
void Player::SendActionButtons(uint32 state) const
{
    ...
    /*
        state can be 0, 1, 2
        0 - Looks to be sent when initial action buttons get sent, however on Darkcore we use 1 since 0 had some difficulties
        1 - Used in any SMSG_ACTION_BUTTONS packet with button data on Darkcore. Only used after spec swaps on retail.
...
void Player::SendMessageToSetInRange(WorldPacket* data, float dist, bool self, bool own_team_only)
{
    ...
    Darkcore::MessageDistDeliverer notifier(this, data, dist, own_team_only);
    VisitNearbyWorldObject(dist, notifier);
}
...
int32 Player::CalculateReputationGain(uint32 creatureOrQuestLevel, int32 rep, int32 faction, bool for_quest, bool noQuestBonus)
{
    ...
    if (rate != 1.0f && creatureOrQuestLevel <= Darkcore::XP::GetGrayLevel(getLevel()))
        percent *= rate;
...
bool Player::RewardHonor(Unit *uVictim, uint32 groupsize, int32 honor, bool pvptoken)
{
    ...
    if (honor_f <= 0)
    {
        ...
        if (uVictim->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            uint8 k_grey = Darkcore::XP::GetGrayLevel(k_level);
            ...
            honor_f = ceil(Darkcore::Honor::hk_honor_at_level_f(k_level) * (v_level - k_grey) / (k_level - k_grey));
...
uint32 Player::_GetCurrencyWeekCap(const CurrencyTypesEntry* currency) const
{
    ...
    case CURRENCY_TYPE_JUSTICE_POINTS:
        {
            uint32 justicecap = sWorld->getIntConfig(CONFIG_MAX_CONQUEST_POINTS) * PLAYER_CURRENCY_PRECISION;
            if (justicecap > 0)
                cap = justicecap;
            break;
        }
...
bool Player::CanCompleteQuest(uint32 quest_id)
{
    if (quest_id)
    {
        ...
        {
            if (qInfo->HasFlag(QUEST_DARKCORE_FLAGS_DELIVER))
            {
            ...
            if (qInfo->HasFlag(QUEST_DARKCORE_FLAGS_KILL_OR_CAST | QUEST_DARKCORE_FLAGS_SPEAKTO))
            {
            ...
            if (qInfo->HasFlag(QUEST_DARKCORE_FLAGS_PLAYER_KILL))
                if (qInfo->GetPlayersSlain() != 0 && q_status.PlayerCount < qInfo->GetPlayersSlain())
                    return false;

            if (qInfo->HasFlag(QUEST_DARKCORE_FLAGS_EXPLORATION_OR_EVENT) && !q_status.Explored)
                return false;

            if (qInfo->HasFlag(QUEST_DARKCORE_FLAGS_TIMED) && q_status.Timer == 0)
                return false;
...
bool Player::CanCompleteRepeatableQuest(Quest const *quest)
{
    ...
    if (quest->HasFlag(QUEST_DARKCORE_FLAGS_DELIVER))
        for (uint8 i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; i++)
...
void Player::AddQuest(Quest const *quest, Object *questGiver)
{
    ...
    if (quest->HasFlag(QUEST_DARKCORE_FLAGS_KILL_OR_CAST | QUEST_DARKCORE_FLAGS_SPEAKTO))
    {
        for (uint8 i = 0; i < QUEST_OBJECTIVES_COUNT; ++i)
            questStatusData.CreatureOrGOCount[i] = 0;
    }

    if (quest->HasFlag(QUEST_DARKCORE_FLAGS_PLAYER_KILL))
    ...
    uint32 qtime = 0;
    if (quest->HasFlag(QUEST_DARKCORE_FLAGS_TIMED))
    {
...
{
    if (!_timedquests.empty() && qInfo->HasFlag(QUEST_DARKCORE_FLAGS_TIMED))
    {
        if (msg)
...
// not used in Darkcore, but used in scripting code
...
//not used in Darkcored, function for external script library
...
void Player::ItemAddedQuestCheck(uint32 entry, uint32 count)
{
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        if (!qInfo || !qInfo->HasFlag(QUEST_DARKCORE_FLAGS_DELIVER))
            continue;
...
void Player::KilledMonsterCredit(uint32 entry, uint64 guid)
{
    ...
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        {
            if (qInfo->HasFlag(QUEST_DARKCORE_FLAGS_KILL_OR_CAST))
            {
                for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
                {
...
bool Player::LoadFromDB(uint32 guid, SQLQueryHolder *holder)
{
    ...
    _LoadIntoDataField(fields[58].GetCString(), PLAYER_EXPLORED_ZONES_1, PLAYER_EXPLORED_ZONES_SIZE);
    _LoadIntoDataField(fields[60].GetCString(), PLAYER__FIELD_KNOWN_TITLES, KNOWN_TITLES_SIZE*2);
    ...
    _currentPetSlot = (PetSlot)fields[62].GetUInt8();
    _petSlotUsed = fields[63].GetUInt32();
    ...
    uint32 instanceId = fields[55].GetUInt32();
...
#define RelocateToHomebind(){ mapId = _homebindMapId; instanceId = 0; Relocate(_homebindX, _homebindY, _homebindZ); }
    ...
    uint32 arena_currency = GetCharacterReputationGuildRep(guid);
    ...
    SetUInt32Value(PLAYER_FIELD_LIFETIME_HONORABLE_KILLS, fields[38].GetUInt32());
    ...
    _specsCount = fields[56].GetUInt8();
    _activeSpec = fields[57].GetUInt8();
    ...
    // RaF stuff.
    _grantableLevels = fields[64].GetUInt32();
    if (GetSession()->IsARecruiter() || (GetSession()->GetRecruiterId() != 0))
        SetFlag(UNIT_DYNAMIC_FLAGS, UNIT_DYNFLAG_REFER_A_FRIEND);
    ...
    if (result)
    {
        do
        {
            ...
            if (quest)
            {
                ...
                if (quest->HasFlag(QUEST_DARKCORE_FLAGS_TIMED) && !GetQuestRewardStatus(quest_id))
                {
                    AddTimedQuest(quest_id);
        ...
        if (LevelMin || LevelMax || missingItem || missingQuest || missingAchievement)
        {
            if (report)
            {
                if (missingQuest && !ar->questFailedText.empty())
                    ...
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetCoreString(LANG_LEVEL_MINREQUIRED_AND_ITEM), LevelMin, sObjectMgr->GetItemTemplate(missingItem)->Name1.c_str());
                else if (LevelMin)
...
void Player::UpdateVisibilityForPlayer()
{
    ...
    Darkcore::VisibleNotifier notifier(*this);
...
{
    uint8 v_level = victim->getLevel();
    uint8 k_grey  = Darkcore::XP::GetGrayLevel(getLevel());
...
uint32 Player::GetCharacterReputationGuildRep(uint32 guid)
{
    PreparedStatement *stmt = CharacterDatabase.GetPreparedStatement(CHAR_GET_CP_VAL);
    stmt->setUInt32(0, guid);
    PreparedQueryResult result = CharacterDatabase.Query(stmt);
    ...
    {
        Field *fields = result->Fetch();
        uint32 arena_currency = fields[0].GetUInt32();
        return arena_currency;
    }
    else
        return 0;
}
