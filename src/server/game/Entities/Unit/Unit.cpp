...
void Unit::CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32* absorb, uint32* resist, SpellInfo const* spellInfo)
{
    ...
    vSchoolAbsorbCopy.sort(Darkcore::AbsorbAuraOrderPred());
    ...
    switch (spellProto->SpellFamilyName)
    {
        case SPELLFAMILY_MAGE:
            ...
            if (spellProto->SpellIconID == 186)
            {
                ...
                if (owner->HasAura(56377)) // Glyph of Ice Lance
                    DoneTotalMod *= 1.05f;
...
Unit* Unit::SelectNearbyTarget(Unit* exclude, float dist) const
{
    ...
    Darkcore::AnyUnfriendlyUnitInObjectRangeCheck u_check(this, this, dist);
    Darkcore::UnitListSearcher<Darkcore::AnyUnfriendlyUnitInObjectRangeCheck> searcher(this, targets, u_check);
...
void Unit::UpdateObjectVisibility(bool forced)
{
    ...
    else
    {
        ...
        Darkcore::AIRelocationNotifier notifier(*this);
...
bool Unit::UpdatePosition(float x, float y, float z, float orientation, bool teleport)
{
    ...
    if (!Darkcore::IsValidMapCoord(x, y, z, orientation))
    {
