...
void WorldSession::HandlePetitionQueryOpcode(WorldPacket & recv_data)
{
    ...
    uint32 guildguid;
    uint64 petitionguid;
    recv_data >> guildguid;                                 // in Darkcore always same as GUID_LOPART(petitionguid)
    recv_data >> petitionguid;                              // petition guid
...
void WorldSession::SendPetitionQueryOpcode(uint64 petitionguid)
{
    ...
    data << uint32(GUID_LOPART(petitionguid));              // guild/team guid (in Darkcore always same as GUID_LOPART(petition guid)
